...
const JobFilter = ({ onFilterChange, initialFilters = {} }) => {
      ...
      try {
        ...
        if (response.data) {
          ...
          const categoriesWithAll = [
            { name: "Tất cả" },
            ...response.data.map((cat) => ({ name: cat })),
          ];
  ...
  const isPeriodSelected = (day, period) => {
    return (
      Array.isArray(filters.available) &&
      filters.available.includes(`${day}-${period}`)
    );
  };
  ...
  const isPartTimeSelected = () => {
    return (
      Array.isArray(filters.jobType) && filters.jobType.includes("Part-Time")
    );
  };
      ...
      <div className="job-filter">
        ...
        <div className="filter-section">
          ...
          >
            ...
            <ExpandMoreIcon
              className={`expand-icon ${expanded.jobType ? "" : "rotated"}`}
            />
          </div>
              ...
              {jobForms.map((form, index) => (
                <div className="checkbox-item" key={index}>
                  <input
                    ...
                    id={`form-${index}`}
                    checked={
                      Array.isArray(filters.jobForm) &&
                      filters.jobForm.includes(form.name)
                    }
        ...
        <div className="filter-section">
          ...
          >
            ...
            <ExpandMoreIcon
              className={`expand-icon ${expanded.workTime ? "" : "rotated"}`}
            />
          </div>
                ...
                <div className="checkbox-item" key={index}>
                  <input
                    ...
                    checked={
                      ...
                      filters.jobType.includes(type.name)
                    }
                      ...
                      {isDaySelected(day) && (
                        <div className="double-nested-options">
                          {["sáng", "chiều", "tối"].map(
                            (period, periodIndex) => (
                              <div
                                className="checkbox-item double-nested-item"
                                key={periodIndex}
                              >
                                <input
                                  type="checkbox"
                                  ...
                                  onChange={() =>
                                    handleScheduleChange(day, period)
                                  }
                                />
                                <label
                                  htmlFor={`period-${index}-${periodIndex}`}
                                >
                                  Ca {period}
                                </label>
                              </div>
        ...
        <div className="filter-section">
          <div
            className="section-header"
            onClick={() => toggleSection("jobPosition")}
          >
            <h4 className="section-title">Vị trí công việc</h4>
            <ExpandMoreIcon
              className={`expand-icon ${expanded.jobPosition ? "" : "rotated"}`}
            />
          </div>
